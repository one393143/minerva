...
export const CardPage = ({ players }) => {
  ...
  return React.createElement('div', {}, // Root container (無動畫)
    // 內容區塊 (有動畫)
    React.createElement('div', { className: 'animate-slide-up space-y-6' },
      // Header
          ...
          React.createElement('div', { className: 'flex gap-2 flex-wrap' },
            [
              ...
              React.createElement('button', {
                ...
                className: `px-4 py-2 rounded-full font-black text-xs uppercase tracking-widest transition-all ${filterTier === tier.value
          ...
          React.createElement('div', { className: 'flex gap-2 items-center' },
            ...
            ['grade', 'name', 'number'].map(sort =>
              React.createElement('button', {
                ...
                className: `px-3 py-1 rounded-full font-bold text-xs transition-all ${sortBy === sort
    ...
    // 詳細卡片 Modal（移出動畫容器外，確保 fixed 定位正確）
    selectedPlayer && React.createElement('div', {
      className: 'fixed inset-0 z-50 flex justify-center items-start pt-12 p-4 bg-black/80 backdrop-blur-sm overflow-y-auto',
